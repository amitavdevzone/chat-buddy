      ...
      <>
        {messagesInOrder.map((message) => (
          <div key={message.id} className={`flex w-full ${message.sender_type === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-xl rounded p-4 ${message.sender_type === 'user' ? 'bg-gray-200 dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-600'}`}
            >
              <p>
        ...
        {isStreaming && (
          <div className="flex w-full justify-start">
            <div className="max-w-xl rounded bg-gray-100 p-4 dark:bg-gray-600">
              <p>
                <strong>Assistant:</strong> <span className="markdown-content" dangerouslySetInnerHTML={{ __html: md.render(streamingMessage) }} />
              </p>
            </div>
          </div>
        )}
